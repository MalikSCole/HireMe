...
const reactLessonCatalog = [
  {
    ...
    summary:
      'Build reusable UI functions and pass data through explicit props.',
...
When should a value be a prop? Use a prop when a parent owns the value and a child needs it to render or handle an interaction.`,
  },
  {
    ...
    summary:
      'Model changing UI data with state and update it from user events.',
...
Why can two setCount(count + 1) calls produce only one increment? Both calls may read the same render's count value. Functional updaters receive the latest queued value.`,
  },
  {
    ...
    summary:
      'Synchronize components with systems outside React without creating render loops.',
...
Why can an array index be a dangerous key? After reordering or deletion, the same index can refer to a different logical item, causing React to preserve state on the wrong row.`,
  },
  {
    ...
    summary:
      'Extract reusable stateful behavior without sharing component instances.',
...
test("switches on when clicked", () => {
  ...
  },
  {
    ...
    description:
      'Extract counter state and actions into a reusable custom hook.',
    difficulty: 'medium' as const,
    requirements: [
      'Create a useCounter custom hook',
      'Return count, increment, and reset',
      'Render the current count',
      'Connect both buttons to the hook actions',
    ],
    starterCode: `import { useState } from "react";
...
const questionCatalog = [
  ...
  },
  {
    ...
    options: [
      'To style the item',
      'To identify the same logical item across renders',
      'To make map run faster',
      'To expose the item globally',
    ],
    correctIndex: 1,
    explanation:
      'Keys help React match each prior element with its next version so identity and local state remain attached to the correct item.',
  },
  {
    lessonSlug: 'react-custom-hooks',
    prompt:
...
async function seed() {
  ...
  console.log(
    `Database seeded with ${catalog.length} problems, ${lessonCatalog.length + reactLessonCatalog.length} lessons, ${questionCatalog.length} knowledge checks, and ${reactChallengeCatalog.length} React challenges.`,
  )
}
